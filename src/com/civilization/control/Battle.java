...
public class Battle {
	...
	public int Player_Def_Army(Island I_def) {
		...
		// 判断岛屿的主人是玩家还是中立敌对
		if (I_def.getHostOfIsland().equals("zhongli")) {
			// 判断防守军数量，若大于5个就出5个，少于5个就全部出动
			if (I_def.getEnemy_Num() >= 5) {
				Def_Army_Num = 5;
			} else {
				Def_Army_Num = I_def.getEnemy_Num();
			}
			return I_def.getEnemy_Kinds();
		} else {
	...
	public void Attack(Island I_atk, Island I_def, int atk_Kind, int def_Kind) {
		...
		if (I_def.getHostOfIsland().equals("zhongli")) {
			I_def.setEnemy_Num(I_def.getEnemy_Num() - def_GG_Num);
		} else {
	...
	public void BattleStart(Island I_atk, Island I_def, int atk_Kind, int def_Kind, int Army_Num) {
		Player_Atk_Army(I_atk, I_def, Army_Num);
